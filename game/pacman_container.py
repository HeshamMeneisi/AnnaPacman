...
class game():
    ...
    def DrawNumber(self, number, p):
        ...
        for i in range(0, len(str(number)), 1):
            ...
            screen.blit(self.digit[iDigit], (p[0] + i * 9, p[1]))
    ...
    def MoveScreen(self, np):
        newX = np[0]
        newY = np[1]
        self.screenPixelPos = np
...
class path_finder():
    def __init__(self):
        ...
        # use the Unfold( c ) function to convert a 2D coordinate pair
    ...
    def Unfold(self, c):
        ...
        # this function converts a 2D array coordinate pair c

        # to a 1D-array index, for the object's 1D map array.

        return (c[0] * self.size[1]) + c[1]

    def Set(self, c, newNode):

        # sets the value of a particular map cell (usually refers to a node object)

        self.map[self.Unfold(c)] = newNode

    def GetType(self, c):

        return self.map[self.Unfold(c)].type

    def SetType(self, c, newValue):

        self.map[self.Unfold(c)].type = newValue
    ...
    def GetG(self, c):
        ...
        return self.map[self.Unfold(c)].g

    def GetH(self, c):

        return self.map[self.Unfold(c)].h

    def SetG(self, c, newValue):

        self.map[self.Unfold(c)].g = newValue

    def SetH(self, c, newValue):

        self.map[self.Unfold(c)].h = newValue

    def SetF(self, c, newValue):

        self.map[self.Unfold(c)].f = newValue

    def CalcH(self, c):

        self.map[self.Unfold(c)].h = abs(c[0] - self.end[0]) + abs(c[1] - self.end[0])

    def CalcF(self, c):

        unfoldIndex = self.Unfold(c)

        self.map[unfoldIndex].f = self.map[unfoldIndex].g + self.map[unfoldIndex].h

    def AddToOpenList(self, c):

        self.openList.append(c)

    def RemoveFromOpenList(self, c):

        self.openList.remove(c)

    def IsInOpenList(self, c):

        if self.openList.count(c) > 0:
    ...
    def AddToClosedList(self, c):

        self.closedList.append(c)

    def IsInClosedList(self, c):

        if self.closedList.count(c) > 0:
    ...
    def SetParent(self, c, pc):

        self.map[self.Unfold(c)].parent = pc

    def GetParent(self, c):

        return self.map[self.Unfold(c)].parent
...
class level():
    ...
    def SetMapTile(self, c, newValue):

        self.map[(c[0] * self.lvlWidth) + c[1]] = newValue

    def GetMapTile(self, c):
        row = c[0]
        col = c[1]
    ...
    def CheckIfHitWall(self, posp, c):
        row = c[0]
        col = c[1]
        possiblePlayerX = posp[0]
        possiblePlayerY = posp[1]
        numCollisions = 0
    ...
    def CheckIfHit(self, plp, p, cushion):
        playerX = plp[0]
        playerY = plp[1]
        x = p[0]
        y = p[1]
    ...
    def CheckIfHitSomething(self, plp, c):
        row = c[0]
        col = c[1]
        playerX = plp[0]
        playerY = plp[1]
    ...
    def PrintMap(self):
        ...
        for row in range(0, self.lvlHeight, 1):
            ...
            for col in range(0, self.lvlWidth, 1):
                outputLine += str(self.GetMapTile(c)) + ", "
...
def CheckInputs(action):
    # allow the user to override the ai action
...
def step(action):
    ...
    if thisGame.mode == 5:
        thisGame.DrawNumber(int(thisGame.ghostValue / 2),
